...
class Host:
    def __init__(self, name, cluster_options):
        ...
        try:
            f = open(os.path.join(self.cluster_options['pssh_out_dir'],self.name), 'r')
        except:
    ...
    def guest_processes(self):
        ...
        out = subprocess.getstatusoutput('grep {} {}'.format(self.cluster_options['bin'], os.path.join(self.cluster_options['pssh_out_dir'], self.name)))
        if out[0] == 0:
            processes = []
